...
export async function GET(_req: Request, { params }: Params) {
  ...
  if (hasSupabase()) {
    const mdx = await sbGetContent("videos", slug)
    if (typeof mdx === "string" && mdx.length) {
      return NextResponse.json({ slug, source: "supabase", mdx })
    }
  }

  const filePath = path.join(process.cwd(), "content", "videos", `${slug}.mdx`)
  try {
...
export async function PUT(req: Request, { params }: Params) {
  ...
  if (hasSupabase()) {
    ...
    if (!existing) {
      const result = await sbInsertContent("videos", slug, mdx)
      if (result === "exists") {
        await sbUpdateContent("videos", slug, mdx)
      }
    } else {
      await sbUpdateContent("videos", slug, mdx)
    }
