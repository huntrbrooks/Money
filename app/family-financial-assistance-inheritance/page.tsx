...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const page = config.familyFinancialAssistanceInheritancePage
  const title = page?.seo?.metaTitle?.trim() || page?.title?.trim() || "Family Financial Assistance and Inheritance"
  const description = page?.seo?.metaDescription?.trim() || page?.description?.trim() || ""
  return buildPageMetadata({
    title,
    description,
    path: "/family-financial-assistance-inheritance",
    keywords: ["family financial assistance", "inheritance", "financial abuse", "therapy"],
  })
}

export default async function FamilyFinancialAssistanceInheritancePage() {
  const config = await readSiteConfig()
  const page = config.familyFinancialAssistanceInheritancePage
  if (!page) notFound()
  ...
  return (
    <div className="min-h-screen bg-muted">
      ...
      <Script
        id="family-financial-assistance-service"
        type="application/ld+json"
        strategy="afterInteractive"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(serviceJsonLd) }}
      />
      <Script
        id="family-financial-assistance-faq"
        type="application/ld+json"
        strategy="afterInteractive"
